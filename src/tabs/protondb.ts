...
export async function loadCompatCacheFromBackend(): Promise<void> {
    ...
    try {
        ...
        const backendCache = await call<[], Record<string, CompatCacheEntry>>('get_compat_cache');
        ...
        if (backendCache && typeof backendCache === 'object') {
            ...
            for (const [key, entry] of Object.entries(backendCache)) {
                if (entry) {
                    compatCache.set(key, {
                        ...
                        deckVerified: (entry.deckVerified as DeckVerifiedStatus) || 'unknown',
            ...
            console.log(`[Unifideck] Loaded ${compatCache.size} entries from backend compat cache`);
        } else {
            console.log('[Unifideck] No compat cache data returned from backend');
        }
    } catch (error) {
        console.error('[Unifideck] Error loading compat cache from backend:', error);
    }
}
...
export function meetsGreatOnDeckCriteria(compat: GameCompatInfo | null): boolean {
    ...
    if (compat.deckVerified === 'verified' || compat.deckVerified === 'playable') return true;
